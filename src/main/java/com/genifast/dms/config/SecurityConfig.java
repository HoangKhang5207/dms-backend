...
public class SecurityConfig {
    ...
    private RSAPublicKey accessTokenPublicKey() {
        try {
            String pem = StreamUtils.copyToString(accessTokenPublicResource.getInputStream(), StandardCharsets.UTF_8);
    ...
    private RSAPrivateKey accessTokenPrivateKey() {
        try {
            String pem = StreamUtils.copyToString(accessTokenPrivateResource.getInputStream(), StandardCharsets.UTF_8);
            byte[] der = PemUtils.parseDerFromPem(pem,
            ...
            throw new IllegalStateException(
                    "Cannot read Access Token private key from " + accessTokenPrivateResource, e);
        } catch (GeneralSecurityException e) {
            throw new IllegalStateException(
