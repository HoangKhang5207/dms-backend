...
public class DatabaseInitializer implements CommandLineRunner {
        ...
        private void createAdminUser(Organization organization) {
                if (userRepository.findByEmail("quantri@genifast.edu.vn").isEmpty()) {
                        ...
                        // Chọn solution này:
                        Role adminRole = roleRepository.findByNameAndOrganizationIsNull("SYSTEM_ADMIN")
                                        .orElseThrow(() -> new RuntimeException(
                                                        "Vai trò quản trị viên hệ thống không tồn tại!"));
        ...
        private Map<String, Permission> createPermissions() {
                List<String> permissionNames = Arrays.asList(
                                ...
                                "documents:history", "documents:archive", "documents:restore",
                                "documents:share:readonly",
                                "documents:share:forwardable", "documents:share:timebound", "documents:share:external",
                                "documents:share:orgscope", "documents:submit", "documents:distribute",
                                "documents:publish",
                ...
                for (String name : permissionNames) {
                        if (!existingPermissions.containsKey(name)) {
                                Permission newPermission = Permission.builder().name(name)
                                                .description("Permission for " + name).createdBy("system")
                                                .build();
        ...
        private void createSystemRoles(Map<String, Permission> permissions) {
                // --- ROLE: Quản trị viên ---
                if (roleRepository.findByNameAndOrganizationIsNull("SYSTEM_ADMIN").isEmpty()) {
        ...
        private Map<String, Department> createTestDepartments(Organization organization) {
                ...
                log.info(">>> Đã tạo {} phòng ban test (K.CNTT, P.DTAO, P.TCHC, BGH, BGH.PC, P.LT) <<<",
                                departments.size());
                return departments;
        }
        ...
        private void createOrganizationRolesWithPermissions(Organization org) {
                ...
                List<String> basicEdit = List.of("documents:create", "documents:update", "documents:delete",
                                "documents:upload", "documents:submit", "documents:comment");
                List<String> approveReject = List.of("documents:approve", "documents:reject");
                List<String> adminExtra = List.of("documents:sign", "documents:lock", "documents:unlock",
                                "documents:history", "documents:archive", "documents:restore", "documents:publish",
                                "documents:track", "documents:notify", "documents:report", "documents:export",
                                "audit:log");
                List<String> sharePerms = List.of("documents:share:readonly", "documents:share:forwardable",
                                "documents:share:timebound", "documents:share:orgscope");
                ...
                ensureRole.accept("Nhân viên Lưu trữ",
                                concat(allRead, List.of("documents:archive", "documents:restore")));
        ...
        private void createTestUsers(Organization organization, Map<String, Department> departments,
                ...
                java.util.function.BiConsumer<User, String> attachRole = (u, roleName) -> {
                        Role r = roleRepository.findByNameAndOrganization_Id(roleName, organization.getId())
                                        .orElseGet(() -> roleRepository.findByNameAndOrganizationIsNull(roleName)
                                                        .orElse(null));
                        if (r != null) {
                                u.getRoles().add(r);
                        }
        ...
        private void createTestProjectsAndDocuments(Organization organization, Map<String, Department> departments) {
                ...
                java.util.function.Function<String, com.genifast.dms.entity.ProjectRole> ensureProjectRole = name -> projectRoleRepository
                                .findByProject_IdAndName(project.getId(), name).orElseGet(() -> {
                                        com.genifast.dms.entity.ProjectRole pr = com.genifast.dms.entity.ProjectRole
                                                        .builder()
                                                        .project(project)
                                                        .name(name)
                ...
                // doc-02: PRIVATE + APPROVED, K.CNTT, Kế hoạch đào tạo, recipients [user-tk,
                // user-gv]
                ...
                // doc-03: INTERNAL + APPROVED (BGH), Hợp tác quốc tế, recipients [user-tk,
                // user-pk]
                ...
                // doc-04: LOCKED + APPROVED (P.TCHC), Báo cáo tài chính, recipients [user-ht,
                // user-lt]
