...
public class AttributeExtractor {
    ...
    public SubjectAttributes extractSubjectAttributes(User user) {
        try {
            ...
            Set<String> roles = user.getUserRoles() != null ? user.getUserRoles().stream()
    ...
    public ResourceAttributes extractResourceAttributes(Document document) {
        try {
            ...
            List<Long> recipients = document.getRecipients().stream()
                    .map(User::getId)
    ...
    private Integer determineUserLevel(User user) {
        if (user.getIsAdmin())
            return 10;
        if (user.getIsOrganizationManager())
            return 9;
        if (user.getIsDeptManager())
            return 8;
        ...
        if (roles.contains("HIEU_TRUONG"))
            return 10;
        if (roles.contains("TRUONG_KHOA"))
            return 7;
        if (roles.contains("PHO_KHOA"))
            return 6;
        if (roles.contains("PHO_PHONG"))
            return 6;
        if (roles.contains("CHUYEN_VIEN"))
            return 5;
        if (roles.contains("GIAO_VU"))
            return 4;
        if (roles.contains("CAN_BO"))
            return 3;
        if (roles.contains("VAN_THU"))
            return 3;
        if (roles.contains("PHAP_CHE"))
            return 7;
        if (roles.contains("NHAN_VIEN_LUU_TRU"))
            return 4;
        if (roles.contains("NGUOI_NHAN"))
            return 2;
